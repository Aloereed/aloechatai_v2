...
class OpenAiService {
  ...
  OpenAiService({this.apiKey, this.apiEndpoint});

  // 新建一个设置apiService的函数，用于后期调整
  void setApiService() {
    this.apiService = USE_MOCK_API_SERVICE
      ? MockedApiRepository(this.apiEndpoint??OPENAI_API_URL)
      : ApiRepository(this.apiEndpoint??OPENAI_API_URL);
  }
  ...
  Future<List<String>> getChatModels() async {
    ...
    List<String> modelIds =
        parseModelIds(modelList, pattern: null, antiPattern: null);
    return modelIds;
  }
}
